...
def genCircle(x, y, r, n):
    return [
        (
            math.cos(2 * math.pi / n * i) * r + x,
            math.sin(2 * math.pi / n * i) * r + y,
        )
        for i in range(0, n)
    ]
...
fout.write(
    str(n * 2) + " " * 33 + "# number of vertices in file\n"
...
fout.write(
    str(2 * n + n // 2 + 1) + " " * 37 + "# number of springs in file\n"
)

for x in range(n):
    fout.write(
        f"{x%n} {(x+1)%n} {tenDig(spring_constant)} {tenDig(rest_length)}\n"
    )

for x in range(n):
    fout.write(
        f"{n + (x%n)} {n + (x+1)%n} {tenDig(spring_constant)} {tenDig(rest_length)}\n"
    )

for x in range(-n // 4, n // 4 + 1):
    fout.write(
        f"{x%n} {3*n//2 - x} {tenDig(spring_constant)} {tenDig(rest_length)}\n"
    )
...
# newSmallcirc_1 = smallcirc_1[(7*n_small//8):]
# + smallcirc_1[:(3*n_small//8 + 1)]
