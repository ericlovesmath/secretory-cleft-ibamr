...
class Circle():
    ...
    def __init__(self, x, y, r, n):
        ...
        self.r = r
        self.n = n
        self.coords = [(cos(2*pi/N_CELL*i)*r + x,
    ...
    def rotate(self, angle):
        self.coords = [(cos(2*pi/N_CELL*i + angle)*r + x,
                        sin(2*pi/N_CELL*i+ angle)*r + y)
                        for i in range(0,N_CELL)]
    def gen_spring(self, offset=0):
        return [f"{(x)%self.vert_count + offset} {(x+1)%self.vert_count + offset} "\
...
class Wall():
    ...
    def gen_spring(self, offset=0):
        return [f"{x+offset} {x+offset+1} {b10(SPRING_CONSTANT)} "\
                f"{b10(dist(self.coords[x], self.coords[x+1]))}\n"
                for x in range(self.spring_count)]
    def gen_beam(self, offset=0):
        return [f"{x+offset} {x+1+offset} {x+2+offset} "\
                f"{b10(RIGIDITY)}\n"
...
x1, y1, x2, y2 = 2,1, 2,7
N_CELL = 64                                           # Number of Points
N_WALL = 64                                           # Number of Points
SPRING_CONSTANT = 5000                              # Spring Constant
REST_LENGTH = round(r*sqrt(2-2*cos(2*pi/N_CELL)), 3)  # Rest Length (recommended listed)
...
file_out.write(f"{cell_wall.beam_count + target_cell.beam_count}"\
