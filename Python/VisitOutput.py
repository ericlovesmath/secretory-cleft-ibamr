...
OpenDatabase(
    "~/Desktop/UCI/Simulations/" + simName + "/viz_IB2d/lag_data.visit"
)
...
for fileID in range(TimeSliderGetNStates()):
    ...
    coords = [
        [coordRow[3 * i], coordRow[3 * i + 1]]
        for i in range(len(coordRow) // 3)
    ]
    print(coords)

    # Output
    outputFile = open(
        "./VisExport/Analysis/" + format(fileID, "04") + ".txt", "w"
    )
